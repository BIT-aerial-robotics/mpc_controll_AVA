...
void mpc::init_mpc_fun()
{
	...
	ROS_INFO_STREAM("u:"<<acadoVariables.u[0]<<" "<<acadoVariables.u[1]<<" "<<acadoVariables.u[2]
	<<" "<<acadoVariables.u[3]<<" "<<acadoVariables.u[4]<<" "<<acadoVariables.u[5]);
...
void mpc::mpc_solver()
{
	//kaidi wang rewrite this function at 2021.10.21
}
...
//get input value and publish them and print them
void mpc::get_input()
{
	...
	while (ros::ok())
	{
		...
		if(mpc_node.start_pub_att.data)//if start_pub_att == true, then start the mpc controller
		{
			...
			if(mpc_node.mode_switch.data)
			{
				...
				mpc_node.get_input();//get input and publish, and print control value
				mpc_node.distributor(mpc_node.thrust_u,mpc_node.torques_u);
				mpc_node.output_publish(mpc_node.ang1,mpc_node.ang2,mpc_node.ang3,mpc_node.thu1,mpc_node.thu2,mpc_node.thu3);

				mpc_node.get_state();//get state and then print state value
